...
# Function is called when a portal is placed in a room. Sets the mesh instance.
func _ready():
...
# Sets the linked portal on the clients
@rpc("authority", "call_remote", "reliable")
func set_interactable_on_clients(path):
	interactable = get_node(path)

# Detect when body entered the area and changes the position of the player to the linked portal
