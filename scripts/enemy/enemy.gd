...
func find_closest_player_in_range(nodes_array: Array):
	...
	# for all nodes in the area compare the distance to the previously remembered closest node
...
# this function gets called only once when the node enters the scene tree
func _ready():
...
func _process(delta):
	...
	# check if the targeted node can be attacked. So: is in zonde, doesnt have immunity.
	...
	# if there is a node in the area then follow the cloesst ndoe
...
# this function only gets called when a player enters the detection area of the enemy.
func _on_detection_area_body_entered(body):
	if body.is_in_group("Players"): #check if it is a palyer
		nodes_in_area.append(body)
		set_physics_process(true)
		set_process(true)

# This function only gets called when a player leaves it detection area.
func _on_detection_area_body_exited(body):
	if body.is_in_group("Players"):
		# remove body from remembered bodies in area.
...
# this function gets called when a player enters the enemies hitbox.
func _on_enemy_hitbox_body_entered(body):
	if body.is_in_group("Players"):
		player_in_attack_zone = true

# this function gets called when a player leaves the enemies hitbox.
